...
with gr.Blocks() as demo:
    ...
    gr.Markdown(
        ...
        <h3> 3. Proceed to landmark control from driven video: </h3>
            3.1. Use the "Upload Driven Video" button to upload an driven video (We have tested .mp4 extensions, and other formats compatible with `cv2.VideoCapture` may also be uploaded without causing errors.). <br>
            ...
            3.4. For video-driven landmark generation, our codes are modified based on <a href='https://github.com/OpenTalker/SadTalker'><b>SadTalker</b></a>. Note that while pure landmark-based control of MOFA-Video supports long video generation via the periodic sampling strategy, current version of hybrid control only supports short video generation (25 frames), which means that the first 25 frames of the generated landmark sequences are used to obtain the result.
