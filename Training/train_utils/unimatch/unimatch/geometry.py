...
def flow_warp(feature, flow, mask=False, padding_mode='zeros'):
    ...
    grid = coords_grid(b, h, w).to(flow.device).to(torch.float16) + flow  # [B, 2, H, W]
...
def warp(x, flo, mode='bilinear', padding_mode='zeros'):
    ...
    vgrid = vgrid.permute(0,2,3,1).to(torch.float16)
    output = F.grid_sample(x, vgrid, mode=mode, padding_mode=padding_mode, align_corners=True)
    mask = torch.ones(x.shape).to(x.device).to(torch.float16)
    mask = F.grid_sample(mask, vgrid, mode=mode, padding_mode=padding_mode, align_corners=True)
